-- Script SQL para crear las tablas del sistema CRM con Showy
-- Ejecutar en Supabase SQL Editor

-- 11. Tabla de conversaciones con Showy
...
-- 12. Tabla de leads generados por conversaciones
...
-- 13. Tabla de propiedades mostradas en conversaciones
...
CREATE POLICY "realtors_can_view_own_conversations" ON conversations
  FOR SELECT USING (realtor_id = auth.jwt() ->> 'realtor_id'::text);

CREATE POLICY "realtors_can_insert_own_conversations" ON conversations
  FOR INSERT WITH CHECK (realtor_id = auth.jwt() ->> 'realtor_id'::text);

CREATE POLICY "realtors_can_update_own_conversations" ON conversations
  FOR UPDATE USING (realtor_id = auth.jwt() ->> 'realtor_id'::text);
